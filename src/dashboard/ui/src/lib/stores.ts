...
export const orderedRepos = derived(
  [availableRepos, pinnedRepos, repoSearchQuery],
  ([$repos, $pinned, $query]) => {
    const filtered = $query
      ? $repos.filter(r => r.repo.toLowerCase().includes($query.toLowerCase()))
      : $repos;
    const pinned = filtered.filter(r => $pinned.includes(r.repo))
...
export const memoriesTotalPages = derived(
  [memoriesTotal, memoriesPageSize],
