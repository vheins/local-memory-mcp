...
export class ArenaRenderer {
	...
	private drawRoom(zone: ZoneRect, isDark: boolean) {
		...
		switch (id) {
			case "in_progress":
				this.drawPlazaFloor(ctx, x, y, w, h, isDark);
				break;
			case "backlog":
				this.drawDirtFloor(ctx, x, y, w, h, "#5b3a6e", isDark);
				break;
			case "pending":
				this.drawDirtFloor(ctx, x, y, w, h, "#a68246", isDark);
				break;
			case "blocked":
			...
			case "burnout":
				...
				break;
			case "completed":
				this.drawGrassFloor(ctx, x, y, w, h, isDark);
				break;
			default:
				this.drawWoodPlankFloor(ctx, x, y, w, h, isDark);
				break;
		}
	...
	private drawRoomDecor(ctx: CanvasRenderingContext2D, zone: ZoneRect, isDark: boolean) {
		...
		switch (id) {
			case "in_progress":
				this.decorWorkspace(ctx, x, y, w, h, isDark);
				break;
			case "backlog":
				this.decorArchive(ctx, x, y, w, h, isDark);
				break;
			case "pending":
				this.decorLobby(ctx, x, y, w, h, isDark);
				break;
			case "blocked":
	...
	private decorGarden(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, isDark: boolean) {
		...
		ctx.beginPath();
		ctx.arc(x + w - 20, y + h - 20, 15, 0, Math.PI * 2);
		ctx.fill();
	}
	...
	private decorIssues(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
	...
	private drawClock(ctx: CanvasRenderingContext2D, cx: number, cy: number, isDark: boolean) {
		...
		for (let i = 0; i < 12; i++) {
			...
			ctx.lineTo(cx + Math.cos(a) * (r2 - 2), cy + Math.sin(a) * (r2 - 2));
			ctx.stroke();
		}
	...
	private drawMonitorActivity(task: VisualTask, sx: number, sy: number, sw: number, sh: number, color: string, isDark: boolean, ts: number) {
		...
		if (active) {
			...
			// Small app chrome and split working panes.
	...
	private drawClaimLinks(scene: ArenaScene, ts: number) {
		...
		for (const a of scene.agents.values()) {
			for (const tid of a.claimedTaskIds) {
				...
				ctx.lineWidth = 1.5;
				ctx.setLineDash([5, 5]);
				ctx.lineDashOffset = -(ts * 0.022) % 10;
				ctx.beginPath();
				ctx.moveTo(a.x, a.y);
				ctx.lineTo(t.x, t.y);
				ctx.stroke();
