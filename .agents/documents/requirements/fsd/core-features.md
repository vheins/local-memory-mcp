...
> **VERIFIED vs IMPLEMENTATION (2026-08-08):** feature areas all shipped, but the tool names below are legacy. Canonical mapping (20 tools): §1 memory — `memory-write` (store/update/acknowledge/supersedes), `memory-read` (search/detail/recap incl. time-tunnel), `memory-delete`; `memory-synthesize` → `synthesize` (Agent Context), `memory-summarize` → `repo-summarize`. §2 task — `task-write` (create/update/interactive/bulk), `task-read` (list/search/detail), `task-delete`; claims/handoffs → `claim-manage`, `handoff-write`, `handoff-read`. §3 standards — `standard-write`/`standard-read`/`standard-delete`. §4 KG — **no MCP KG tools**; entities/relations/observations are dashboard/API + embedded read context (ADR-006); auto-extraction runs via the embedding outbox worker, not inline on write. §5 agent tools — `agent-context` only; `decision-log`/`session-summarize` absorbed into `memory-write`. §6 resources verified (`repository://{name}/memories`, `/tasks`, `/summary`, `/actions`, `memory://{id}`, `repository://index` — src/mcp/resources/sdk-index.ts); "31 prompt templates" → **32** shipped (src/mcp/prompts/definitions). §7 dashboard tabs — shipped tabs include **Arena**, **Codebase**, and **Queue** (11 tabs; Queue added 2026-08-09, TASK-297). §9 soul maintenance ✓ (7-day decay, 0.5 rate, min importance 1, immunized tags, archiving). Collision threshold in §1 "> 0.55" → shipped **0.85** (`MEMORY_CONFLICT_THRESHOLD`).
