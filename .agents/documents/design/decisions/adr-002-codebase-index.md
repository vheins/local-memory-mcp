...
> **VERIFIED vs IMPLEMENTATION (2026-08-08):** decision in force. Corrections: (1) Decision 2 "4 new tables (`codebase_files`, `codebase_symbols`, `codebase_relations`, `codebase_index_queue`)" — shipped DDL is `codebase_files`+`codebase_symbols` (migration v01) and `codebase_references` (v21); there is **no** `codebase_relations` and **no** `codebase_index_queue` table (queueing uses `queue_jobs` v09). (2) Migration system is at v22 (battle-tested through v1→v22). (3) Decision 3 SHA-256 checksum incremental — implemented (mtime/checksum short-circuit + `skippedByMtime`). (4) Decision 4 single-pass symbols + references pass ships via reference emission at parse time (ts-reference-emission.ts); dead-code analysis is **NEXT PHASE**. (5) Language matrix: 15 languages shipped (typescript/tsx/vue/go/python/php/dart/rust/java/ruby/kotlin/swift/c/cpp/markdown) — matches the v0.21.0 note in Consequences.
...
- **Multi-language parsing**: Go, Python, PHP, Rust, Java, Dart, Kotlin, Ruby, Swift, C, and C++ visitors added in v0.21.0. See [Feature Overview](../../../../docs/features/codebase-index.md) for the full language matrix.
