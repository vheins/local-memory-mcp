...
>
> **VERIFIED vs IMPLEMENTATION (2026-08-10, Phase 1.1 + dead-code shipped):** `codebase_references` is now a generalized **edge table** — migration **v23** added `target_file` + `target_symbol_id` (nullable, plain-TEXT pointers) and extended `kind` to `call` \| `instantiation` \| `import` \| `extends` \| `implements` (`src/mcp/storage/migrations/v23-codebase-references-edge-targets.ts:1-49`, TASK-299). SCHEMA_VERSION is now **24** (`src/mcp/storage/migrations/index.ts:29`; v24 `relations-confidence`, TASK-325, is KG-only). Dead-code/hotspots are computed per ARCHITECTURE query (`src/mcp/codebase-index/services/dead-code.ts`) — **no** denormalized counter column was added. `parent_symbol_id` hierarchy ships in `codebase_symbols` (v01 column, populated at index time, TASK-300).
