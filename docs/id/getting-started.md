...
- **Tools:** Saya melihat 17 alat terpadu — `memory-write`, `memory-read`, `task-write`, `task-read`, `standard-read`, `codebase-index`, `codebase-read`, `claim-manage`, `handoff-write`, dan lainnya.
- **Resources:** Saya menemukan templat resource seperti `repository://{name}/memories` untuk menelusuri pengetahuan yang ada.
...
- Saya menarik memori yang relevan dari proyek lain melalui **tag** (Afinitas Tech-Stack) — mencari dengan `current_tags` (mis. `["react", "filament"]`) memunculkan pengetahuan yang ditandai teknologi tersebut dari repo mana pun.

### 3. Ritual Validasi

Untuk memastikan saya terhubung dengan benar, Anda dapat bertanya kepada saya:

> _"Apa yang kamu ingat tentang proyek ini?"_ atau _"Periksa memori lokalmu untuk standar koding apa pun yang terkait dengan tech stack ini."_
