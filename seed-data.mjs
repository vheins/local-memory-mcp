		...
		const memories = [
			{
				type: "decision",
				content:
					"Use TypeScript for all new backend services. Static typing reduces bugs and improves maintainability.",
				importance: 5,
				scope: { owner: "", repo: "backend-api" }
			},
			{
				type: "mistake",
				content:
					"Don't use ORM queries in loops - causes N+1 query problem. Always use batch loading or raw SQL with joins.",
				importance: 4,
				scope: { owner: "", repo: "backend-api", language: "typescript" }
			},
			{
				type: "code_fact",
				content:
					"Authentication is handled by JWT middleware in src/middleware/auth.ts. Token validation uses RS256 algorithm.",
				importance: 3,
				scope: { owner: "", repo: "backend-api", folder: "src/middleware" }
			},
			{
				type: "pattern",
				content:
					"All API responses follow the format: { success: boolean, data?: any, error?: string }. Maintain consistency.",
				importance: 4,
				scope: { owner: "", repo: "backend-api" }
			}
		];
